...
TQW.Config = {
    ...
    Negotiator = {
        Enabled       = true,
        Range         = 3,      -- casillas
        PauseMs       = 5000,   -- pausa de hostilidades
        CooldownMs    = 300000, -- 5 min por bandido
        IntervalMs    = 1000,
    },
...
--[[ #7 - EL NEGOCIADOR / EXTORSIONADOR --------------------------------------
     ...
     bandido baja el arma unos segundos y exige el equipo.
...
local function Feature_Negotiator(bandit, brain, state, now)
    ...
    -- Restauracion pendiente de hostilidad (se comprueba siempre, sin intervalo)
    if brain.tqwHostileUntil and now >= brain.tqwHostileUntil then
        ...
        Log("Negotiator: hostilidad restaurada")
    end
    ...
    brain.tqwHostileUntil = now + cfg.PauseMs
    ...
    TQW.Chat(bandit, Choice(TQW.Lines.negotiate))
    Log("Negotiator: extorsionando al jugador")
end
